...
namespace GT.RText_JP
{
    public partial class Main : Form
    {
        ...
        public Main()
        {
            ...
            listViewPages.Columns.Add("カテゴリー", -2, HorizontalAlignment.Left);
        ...
        private void saveToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                if (_isUiFolderProject)
                {
                    ...
                    toolStripStatusLabel.Text = $"{saveFileDialog.FileName} - {_rTexts.Count}つの言語にデータが保存されました";
                }
                else
                {
                    ...
                    toolStripStatusLabel.Text = $"{saveFileDialog.FileName} - 保存完了";
                }
            }
            catch (Exception ex)
            {
        ...
        private void listViewCategories_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                toolStripStatusLabel.Text = $"{page.Name} - {page.PairUnits.Count} アイテム";
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
            ...
            try
            {
                ...
                if (rowEditor.ShowDialog() == DialogResult.OK)
                {
                    if (_isUiFolderProject && rowEditor.ApplyToAllLocales)
                    {
                        ...
                        toolStripStatusLabel.Text = $"{rowEditor.Label} - {_rTexts.Count}つの言語にデータが編集されました";
                    }
                    else
                    {
                        if (rowEditor.Label != rowEditor.Label && page.PairExists(rowEditor.Label))
                        {
                            MessageBox.Show("このラベルはこのカテゴリにすでに存在します.", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            return;
                        }
                        ...
                        toolStripStatusLabel.Text = $"{rowEditor.Label} - 編集完了";
                    }
            ...
            try
            {
                ...
                if (rowEditor.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (_isUiFolderProject && rowEditor.ApplyToAllLocales)
                    {
                        ...
                        toolStripStatusLabel.Text = $"{rowEditor.Label} - {_rTexts.Count}つの言語にデータが追加されました";
                    }
                    else
                    {
                        var rowId = page.AddRow(rowEditor.Id, rowEditor.Label, rowEditor.Data);
                        toolStripStatusLabel.Text = $"{rowEditor.Label} - 追加完了";
                    }
        ...
        private void removeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (MessageBox.Show($"「{rowData.Label}」を削除してもよろしいですか？", "確認", MessageBoxButtons.YesNoCancel,
                    MessageBoxIcon.Information) == DialogResult.Yes)
                {
                    page.DeleteRow(rowData.Label);

                    toolStripStatusLabel.Text = $"{rowData.Label} - 削除完了";
        ...
        private void addEditFromCSVFileToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                toolStripStatusLabel.Text = $"CSVファイルを読み込めません: {ex.Message}";
                return;
            }

            if (!kv.Any())
            {
                toolStripStatusLabel.Text = "エラー: CSVファイルに有効なキーと値のペアが見つかりません。";
                return;
            }

            if (_isUiFolderProject)
            {
                var res = MessageBox.Show($"現在開いているすべての言語に追加しますか?", "確認", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Information);
                if (res == DialogResult.Yes)
                {
                    ...
                    toolStripStatusLabel.Text = $" {_rTexts.Count}言語、{kv.Count}エントリが追加/削除されました";
                }
                else if (res == DialogResult.No)
                {
                    CurrentPage.AddPairs(kv);
                    toolStripStatusLabel.Text = $"{kv.Count}エントリが追加/削除されました";
                }
                else
                    return;
            }
        ...
        private RTextParser ReadRTextFile(string filePath)
        {
            ...
            {
                toolStripStatusLabel.Text = $"ファイルの読み込みエラー: {filePath}";
                MessageBox.Show("Couldn't decrypt all strings. Please contact xfileFIN for more information.", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            catch (Exception ex)
            {
                toolStripStatusLabel.Text = $"ファイルの読み込みエラー: {filePath}";
        ...
        private void DisplayPages()
        {
            ...
            {
                MessageBox.Show("最初に有効なRT04ファイルを読み込む必要があります。", "", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
